...
[Unit]
Description=Finance App Django Application
...
ExecStart=$PROJECT_PATH/venv/bin/gunicorn finance_project.wsgi:application --bind 0.0.0.0:8000 --workers 4
...
install_service() {
    ...
    log_info "启用服务（开机自启）..."
    systemctl enable finance-app.service
    
    log_info "启动服务..."
    systemctl start finance-app.service
    
    log_success "服务安装并启动完成"
}
...
show_status() {
    ...
    echo ""
    log_success "==================== 常用命令 ===================="
    log_info "查看状态: sudo systemctl status finance-app"
    log_info "启动服务: sudo systemctl start finance-app"
    log_info "停止服务: sudo systemctl stop finance-app"
    log_info "重启服务: sudo systemctl restart finance-app"
    log_info "查看日志: sudo journalctl -u finance-app -f"
    log_info "取消自启: sudo systemctl disable finance-app"
    log_success "======================================================="
}
